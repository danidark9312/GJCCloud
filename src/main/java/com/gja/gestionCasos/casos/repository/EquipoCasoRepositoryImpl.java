...
public class EquipoCasoRepositoryImpl extends AbstractRepository<EquipoCaso> implements EquipoCasoRepository {
	...
	public List<Caso> obtenerCasosAbogado(User abogado) throws DAOException,BusinessException{
		List<Caso> abogados = null;
		abogados = entityManager.createQuery("SELECT c FROM Caso c join fetch c.casoEquipoCasoSet cec  WHERE cec.equipoCaso.usuario.id= :pAbogado and cec.activo = :pEstado")
				.setParameter("pAbogado", abogado.getId())
				.setParameter("pEstado", "S")
				.getResultList();
		return abogados;
