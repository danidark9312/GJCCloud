...
<!-- 													</div> -->
																	...
																	<div class="row">
																		<div class="col-sm-12 col-md-6">
																			<h3 class="titulo-azul">
																				<spring:message code="label.title.tabla.archivos" />
																			</h3>
																		</div>
																		<div class="hidden-sm hidden-xs col-md-6 form-group btns-forms text-right">
																			<label> </label>
																			<button class="btn btn-azul" type="button" onclick="listaArchivos.mostrarModalAsociarArchivo()" style="margin-top:0;">
			...
			<!-- Ventanas modales -->
			<%@ include file="/WEB-INF/jsp/archivos/modalAsociarArchivo.jsp" %>
...
// 							var token = $("#_csrf").val();
							...
							var cdusuarioLogeado = $("#idusercreacion").val()
 							myDropzone = new Dropzone("#dropzone_asociarArchivo", {
 								url : contexto + "/listaArchivos/asociarArchivo?" + "_csrf=" + $("#_csrf").val() + "&cdusuarioLogeado="
 										+ $("#idusercreacion").val(),
 								dictDefaultMessage : "Para subir arrastre un archivo u oprima ",
 								addRemoveLinks : true,
 								dictRemoveFile : 'Remover',
 								acceptedFiles : ".xls,.xlsx,.pdf,.PDF,.doc,.docx,.txt,.ppt,.pot",
 								maxFilesize : 30,
 								maxFiles : 1,
 								parallelUploads : 1,
